...
Public Class Form1
    ...
    Dim DP, inpDP, ToothNo, Nmod, Cmod, Td, CDP, Tn, Sd, Sn, ThetaR, CPAr, NPAr, temp1, convert, Rad, Pitch, inpToothNo, DoP, inpDoP, AlphaR, PHA, inpPHA, PHAr, PA, NPA, CPA, PAr, inpASW, ASW, BCD, inpPA, ArcTTh, TranTTh, inpArcTTh, PCD, inpPCD, MPD, inpMPD, Theta, Rb, Rt, Ri, Dbase, Pang, MPD1, Doe, Beta, BetaR, H, Hr, Alpha, Twoc, Eo, Vol As Double
    ...
    Sub Opft() 'FUNCTION 1
        ...
        CDP = inpDP * Cos(PHAr)
        PCD = ToothNo / CDP
        PCD = FormatNumber(PCD, Deci)
        If ComboPA.SelectedItem = "NPA" Then
            NPA = inpPA
            NPAr = NPA * PI / 180
            '  CPA = Atan(Tan(inpPA) / Cos(Beta))
            '  CPA = Atan(Tan(NPAr / Cos(PHAr)))
            CPA = Atan(Tan(NPAr) / Cos(PHAr))
            CPAr = CPA * 180 / PI
            CPAr = FormatNumber(CPAr, Deci)
            MsgBox(CPAr)
        ...
        If Beta > 0.000001 Then
            'Was PA
            H = Atan(Tan(BetaR) * Cos(PA))

            MPD = inpMPD / Cos(H)
        End If
        '  MsgBox(PCD)
        BCD = PCD * (Cos(CPAr / Rad))
        BCD = FormatNumber(BCD, Deci)
        MsgBox(BCD)

        Vol = TranTTh / PCD + MPD / BCD + (Tan(CPA) - CPA) - PI / ToothNo
        '  MsgBox("Vol = " & Vol)
        Alpha = Ainv(Vol)
        Twoc = BCD / Cos(Alpha)
        Eo = ToothNo Mod 2
    ...
    Sub Upfs() 'Function 2
        ...
        If RadioFunc2.Checked = True Then
            PanelF3Results.Visible = False
            PanelF4Results.Visible = False
            ResultsPanel.Visible = True
        End If
    ...
    Sub Sfup() 'Function 4
        ...
        Sd = PCD * (Tan(Ang) - Ang + MPD / BCD - (Tan(PAr) - PAr))
        Sn = Sd * Cos(BetaR)
